...
class tpat_test_conv_with_autopad_same: public IPluginV2DynamicExt {
public:
    tpat_test_conv_with_autopad_same() {}
    
    tpat_test_conv_with_autopad_same(const void *buffer, size_t length) {
    }
    ...
    nvinfer1::IPluginV2DynamicExt* clone() const noexcept override {
        return new tpat_test_conv_with_autopad_same();
    }
    ...
    const char* getPluginType() const noexcept override {return "tpat_test_conv_with_autopad_same";}
...
class tpat_test_conv_with_autopad_sameCreator: public nvinfer1::IPluginCreator {
public:
    tpat_test_conv_with_autopad_sameCreator(){
    ...
    nvinfer1::IPluginV2DynamicExt* deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept override {
        tpat_test_conv_with_autopad_same* obj = new tpat_test_conv_with_autopad_same{serialData, serialLength};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
    
    const char* getPluginName() const noexcept override {return "tpat_test_conv_with_autopad_same";}
    const char* getPluginVersion() const noexcept override {return "1";}
    ...
    nvinfer1::IPluginV2DynamicExt* createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept override {
        //std::cout << __FUNCTION__ << std::endl;
        tpat_test_conv_with_autopad_same* obj = new tpat_test_conv_with_autopad_same{};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
private:
