...
class tpat_tpat_split: public IPluginV2DynamicExt {
public:
    tpat_tpat_split() {}
    
    tpat_tpat_split(const void *buffer, size_t length) {
    }
    ...
    nvinfer1::DimsExprs getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs, int nbInputs, nvinfer1::IExprBuilder& exprBuilder) noexcept override {
        ...
        if (outputIndex == 0){
            ...
            output_shape.d[1] = exprBuilder.constant(64);
    ...
    const char* getPluginType() const noexcept override {return "tpat_tpat_split";}
...
class tpat_tpat_splitCreator: public nvinfer1::IPluginCreator {
public:
    tpat_tpat_splitCreator(){
    ...
    nvinfer1::IPluginV2DynamicExt* deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept override {
        tpat_tpat_split* obj = new tpat_tpat_split{serialData, serialLength};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
    
    const char* getPluginName() const noexcept override {return "tpat_tpat_split";}
    const char* getPluginVersion() const noexcept override {return "1";}
    ...
    nvinfer1::IPluginV2DynamicExt* createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept override {
        //std::cout << __FUNCTION__ << std::endl;
        tpat_tpat_split* obj = new tpat_tpat_split{};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
private:
