...
class tpat_test_scatternd: public IPluginV2DynamicExt {
public:
    tpat_test_scatternd() {}
    
    tpat_test_scatternd(const void *buffer, size_t length) {
    }
    ...
    nvinfer1::IPluginV2DynamicExt* clone() const noexcept override {
        return new tpat_test_scatternd();
    }
    ...
    nvinfer1::DimsExprs getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs, int nbInputs, nvinfer1::IExprBuilder& exprBuilder) noexcept override {
        ...
        if (outputIndex == 0){
            ...
            output_shape.d[2] = exprBuilder.constant(256);
    ...
    size_t getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs, int nbInputs, const nvinfer1::PluginTensorDesc* outputs, int nbOutputs) const noexcept override{
        return 4096 + 256 * 128 * 256 * sizeof(float) + 256 * 2 * sizeof(int) + 256 * 256 * sizeof(float) + 256 * 128 * 256 * sizeof(float);
    }
    ...
    const char* getPluginType() const noexcept override {return "tpat_test_scatternd";}
...
class tpat_test_scatterndCreator: public nvinfer1::IPluginCreator {
public:
    tpat_test_scatterndCreator(){
    ...
    nvinfer1::IPluginV2DynamicExt* deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept override {
        tpat_test_scatternd* obj = new tpat_test_scatternd{serialData, serialLength};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
    
    const char* getPluginName() const noexcept override {return "tpat_test_scatternd";}
    const char* getPluginVersion() const noexcept override {return "1";}
    ...
    nvinfer1::IPluginV2DynamicExt* createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept override {
        //std::cout << __FUNCTION__ << std::endl;
        tpat_test_scatternd* obj = new tpat_test_scatternd{};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
private:
