...
class tpat_test_lstm_without_bias: public IPluginV2DynamicExt {
public:
    tpat_test_lstm_without_bias() {}
    
    tpat_test_lstm_without_bias(const void *buffer, size_t length) {
    }
    ...
    nvinfer1::IPluginV2DynamicExt* clone() const noexcept override {
        return new tpat_test_lstm_without_bias();
    }
    ...
    nvinfer1::DimsExprs getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs, int nbInputs, nvinfer1::IExprBuilder& exprBuilder) noexcept override {
        ...
        if (outputIndex == 0){
            ...
            output_shape.d[1] = exprBuilder.constant(2);
            output_shape.d[2] = exprBuilder.constant(1);
            output_shape.d[3] = exprBuilder.constant(32);
        ...
        if (outputIndex == 1){
            ...
            output_shape.d[2] = exprBuilder.constant(32);
    ...
    const char* getPluginType() const noexcept override {return "tpat_test_lstm_without_bias";}
...
class tpat_test_lstm_without_biasCreator: public nvinfer1::IPluginCreator {
public:
    tpat_test_lstm_without_biasCreator(){
    ...
    nvinfer1::IPluginV2DynamicExt* deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept override {
        tpat_test_lstm_without_bias* obj = new tpat_test_lstm_without_bias{serialData, serialLength};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
    
    const char* getPluginName() const noexcept override {return "tpat_test_lstm_without_bias";}
    const char* getPluginVersion() const noexcept override {return "1";}
    ...
    nvinfer1::IPluginV2DynamicExt* createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept override {
        //std::cout << __FUNCTION__ << std::endl;
        tpat_test_lstm_without_bias* obj = new tpat_test_lstm_without_bias{};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
private:
