...
class tpat_test_lstm: public IPluginV2DynamicExt {
public:
    tpat_test_lstm() {}
    
    tpat_test_lstm(const void *buffer, size_t length) {
    }
    ...
    nvinfer1::DimsExprs getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs, int nbInputs, nvinfer1::IExprBuilder& exprBuilder) noexcept override {
        ...
        if (outputIndex == 0){
            ...
            output_shape.d[2] = inputs[0].d[1];
            output_shape.d[0] = exprBuilder.constant(2);
            output_shape.d[1] = exprBuilder.constant(1);
            output_shape.d[3] = exprBuilder.constant(4);
    ...
    size_t getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs, int nbInputs, const nvinfer1::PluginTensorDesc* outputs, int nbOutputs) const noexcept override{
        return 1040 + 256 * 2 * 1 * sizeof(float) + 2 * 1 * 256 * 4 * sizeof(float) + 1 * 256 * 4 * sizeof(float) + 1 * 256 * 4 * sizeof(float);
    }
    ...
    const char* getPluginType() const noexcept override {return "tpat_test_lstm";}
...
class tpat_test_lstmCreator: public nvinfer1::IPluginCreator {
public:
    tpat_test_lstmCreator(){
    ...
    nvinfer1::IPluginV2DynamicExt* deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept override {
        tpat_test_lstm* obj = new tpat_test_lstm{serialData, serialLength};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
    
    const char* getPluginName() const noexcept override {return "tpat_test_lstm";}
    const char* getPluginVersion() const noexcept override {return "1";}
    ...
    nvinfer1::IPluginV2DynamicExt* createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept override {
        //std::cout << __FUNCTION__ << std::endl;
        tpat_test_lstm* obj = new tpat_test_lstm{};
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
private:
