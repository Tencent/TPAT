...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        if dynamic == True:
            input_x = tf.placeholder(dtype=tf.float32,shape=[None, 192], name='input_x')
        else:
            input_x = tf.placeholder(dtype=tf.float32,shape=[batch_size, 192], name='input_x')

        x = tf.split(input_x, [64, 128], axis=1, name=op_name)
 
        output1 = tf.identity(x[0], name="output0")
        output2 = tf.identity(x[1], name="output1")
        sess.run(tf.global_variables_initializer())

        x_batch = np.random.rand(batch_size, 192).astype(np.float32)

        input_node = [input_x]
        input_data = [x_batch]
        input_with_num = ["input_x:0"]
        output_with_num = ["output0:0", "output1:0"]
        ...
        #a = datetime.now()
        for i in range(iterations):
            tf_result = sess.run([output1, output2], input_dict)
        #b = datetime.now()
        #time_sum = (b - a).total_seconds()
        #tf_time = "[INFO] TF  execution time " + str(
        #    time_sum * 1000 / iterations) + " ms"

        output_name_without_port = ["output0", "output1"]
